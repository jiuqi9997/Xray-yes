...
information_file="$HOME/xray.inf"
...
install_packages() {
	...
	apt_packages="libcurl4-openssl-dev gcc make zip unzip openssl libssl-dev libxml2 libxml2-dev zlib1g zlib1g-dev libjpeg-dev libpng-dev lsof libpcre3 libpcre3-dev cron net-tools swig build-essential libffi-dev libbz2-dev libncurses-dev libsqlite3-dev libreadline-dev tk-dev libgdbm-dev libdb-dev libdb++-dev libpcap-dev xz-utils git libgd3 libgd-dev libevent-dev libncurses5-dev uuid-dev jq"
	if [[ $PM == "apt-get" ]]; then
		$INS $apt_packages
...
issue_certificate() {
	fail=0
	/root/.acme.sh/acme.sh --issue $xray_domain --keylength ec-256 --fullchain-file "$cert_dir/cert.pem" --key-file "$cert_dir/key.pem" --webroot "$website_dir/$xray_domain" || fail=1
...
install_xray() {
	...
	[[ ! $(ss -tnlp | grep $port) ]] && error "xray 安装失败"
	success "xray 安装成功"
}
...
finish() {
	...
	echo ""
	echo ""
	echo -e "$Red xray 配置信息 $Font" | tee $information_file
	echo -e "$Red 地址（address）:$Font $server_ip " | tee -a $information_file
	echo -e "$Red 端口（port）：$Font $port " | tee -a $information_file
	echo -e "$Red 用户id（UUID/密码）：$Font $uuid" | tee -a $information_file
}
...
prepare_installation() {
	...
	if [[ $ip_type -eq 1 ]]; then
		...
		[[ server_ip == domain_ip ]] && success "域名已经解析到本机" &&
		continue
		...
		*)
		...
		server_ip6=$(curl -sL https://api.ip.sb/ip -6 || fail=1)
		[[ $fail -eq 1 ]] && error "本机ip获取失败"
		[[ $server_ip == $domain_ip ]] && success "域名已经解析到本机" &&
		continue
		warning "域名没有解析到本机，证书申请可能失败"
		...
		*)
		...
		[[ $server_ip == $domain_ip ]] && success "域名已经解析到本机（ipv4）" &&
		continue
		...
		*)
		...
		[[ $server_ip == $domain_ip ]] && success "域名已经解析到本机（ipv6）" &&
		continue
...
update_xray() {
	curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh | bash -s -- install
	[[ ! $(ss -tnlp | grep $port) ]] && error "xray 更新失败"
	success "xray 更新成功"
}
...
mod_uuid() {
	...
	read -rp "请输入xray密码（默认使用uuid）：" uuid
	[[ -z $uuid ]] && uuid=$(xray uuid)
	sed -i "s/$uuid_old/$uuid/g" $xray_conf
...
mod_port() {
	...
	sed -i "s/$port_old/$port/g" $xray_conf
...
xray_restart() {
	...
	[[ ! $(ss -tnlp | grep $port) ]] && error "xray 重启失败"
	success "xray 重启成功"
	sleep 2
	main
}
...
show_configuration() {
	[[ -f $information_file ]] && cat $information_file && exit 0
	panic "配置文件已丢失"
}
